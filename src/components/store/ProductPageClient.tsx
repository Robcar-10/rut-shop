...
export default function ProductPageClient({
  ...
  const related = allProducts
    ...
    .slice(0, 4);
  if (related.length < 4) {
    const padding = allProducts.filter((p) => p.id !== product.id && !related.includes(p)).slice(0, 4 - related.length);
  ...
  return (
    <div>
      ...
      <main className="rut-container" style={{ padding: "40px 32px 0" }}>
        ...
        <div className="product-page-grid">
            ...
            <div className="gallery-thumbs">
              {product.images.map((src, i) => (
                <button key={i} onClick={() => setActiveImg(i)} style={{
                  ...
                  cursor: "pointer", background: "var(--rut-surface-soft)", transition: "border-color .15s", position: "relative",
          ...
          <div className="product-info">
            ...
            <div style={{ marginTop: 20, display: "flex", alignItems: "baseline", gap: 10 }}>
              ...
              {onSale && <span style={{ fontSize: 12, fontWeight: 800, color: "var(--rut-coral)", letterSpacing: ".08em", textTransform: "uppercase", marginLeft: 4 }}>Save ${(product.price - (product.salePrice ?? 0)).toFixed(0)}</span>}
            </div>
            ...
            <div style={{ marginTop: 8 }}>
              ...
              <Accordion title="Shipping & turnaround" open={expanded.ship} onToggle={() => setExpanded((e) => ({ ...e, ship: !e.ship }))}>
                <p style={{ margin: 0, color: "var(--rut-fg-600)", fontSize: 14, lineHeight: 1.7 }}>
                  Orders ship from our Nyack print shop within 3–5 business days. Free shipping on orders over $75 in the lower 48 states.
                </p>
              </Accordion>
              <Accordion title="Returns & exchanges" open={expanded.returns} onToggle={() => setExpanded((e) => ({ ...e, returns: !e.returns }))}>
                <p style={{ margin: 0, color: "var(--rut-fg-600)", fontSize: 14, lineHeight: 1.7 }}>
                  Unworn, unwashed items can be returned within 30 days. We&apos;ll cover return shipping for size exchanges.
                </p>
              </Accordion>
            </div>
          </div>
        ...
        <section style={{ marginTop: 96 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 24 }}>
            <h2 className="h-display" style={{ fontSize: 32 }}>You Might Also <span className="gradient-text">Like</span></h2>
      ...
      <style>{`
        .product-page-grid { display: grid; grid-template-columns: 1.15fr 1fr; gap: 56px; align-items: start; }
        .product-gallery { display: grid; grid-template-columns: 84px 1fr; gap: 16px; }
        .gallery-thumbs { display: flex; flex-direction: column; gap: 12px; }
        .product-info { position: sticky; top: 96px; }
        .related-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 80px; }
        @media (max-width: 1024px) { .product-page-grid { gap: 36px; } .related-grid { grid-template-columns: repeat(3, 1fr); } }
        @media (max-width: 860px) { .product-page-grid { grid-template-columns: 1fr; } .product-info { position: static; } .related-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 600px) { .product-gallery { grid-template-columns: 1fr; } .gallery-thumbs { flex-direction: row; } .gallery-thumbs button { width: 64px !important; height: 80px !important; } .related-grid { grid-template-columns: repeat(2, 1fr); gap: 12px; } }
      `}</style>
    </div>
  );
}
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--rut-border)" }}>
      <button onClick={onToggle} style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 0", background: "transparent", border: 0, cursor: "pointer", fontFamily: "inherit" }}>
        ...
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--rut-fg-500)" strokeWidth="2.2" style={{ transform: open ? "rotate(180deg)" : "rotate(0)", transition: "transform .2s", flexShrink: 0 }}><polyline points="6 9 12 15 18 9" /></svg>
      </button>
