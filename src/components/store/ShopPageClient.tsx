  ...
  return (
    <div>
      ...
      <main className="rut-container" style={{ padding: "40px 32px 0" }}>
        {category === "all" && featured && <FeaturedHero product={featured} />}
      ...
      <style>{`
        .product-grid-hero { display: grid; grid-template-columns: 1.4fr 1fr; gap: 20px; margin-bottom: 24px; }
        .product-grid-rest { display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; margin-bottom: 80px; }
        ...
        @media (max-width: 400px) { .product-grid-rest { grid-template-columns: 1fr; } }
      `}</style>
    </div>
  );
}
...
function ShopHero() {
  return (
    <section style={{ position: "relative", overflow: "hidden", background: "var(--rut-gradient-night-hero)" }}>
      ...
      <div className="rut-container shop-hero-inner">
        <div className="shop-hero-text">
          ...
          <p style={{ color: "rgba(255,255,255,.7)", fontSize: 18, lineHeight: 1.6, marginTop: 22, maxWidth: 520 }}>
            Ready-made merch from our Nyack print shop. Every item is printed by us, in-house, by hand.{" "}
      ...
      <style>{`
        .shop-hero-inner { position: relative; padding: 64px 32px 72px; display: grid; grid-template-columns: 1.4fr .9fr; gap: 56px; align-items: center; }
        .shop-hero-h1 { font-size: 80px; }
        @media (max-width: 900px) { .shop-hero-inner { grid-template-columns: 1fr; gap: 32px; padding: 48px 20px 56px; } .shop-hero-h1 { font-size: 56px; } .shop-hero-card { display: none; } }
        @media (max-width: 480px) { .shop-hero-inner { padding: 36px 16px 44px; } .shop-hero-h1 { font-size: 42px; } .shop-hero-text p { font-size: 15px !important; } }
      `}</style>
    </section>
  );
}
...
function DropBullet({ label, sub }: { label: string; sub: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      <span style={{ width: 28, height: 28, borderRadius: 8, background: "linear-gradient(135deg, rgba(178,33,246,.4), rgba(255,100,82,.4))", border: "1px solid rgba(255,255,255,.15)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
...
function FeaturedHero({ product }: { product: Product }) {
  return (
    <Link href={`/products/${product.id}`} className="featured-hero-card" style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", background: "#fff", border: "1px solid var(--rut-border)", borderRadius: 24, overflow: "hidden", marginBottom: 56, textDecoration: "none" }}>
      ...
      <div style={{ padding: "48px 48px 44px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          <span className="btn-primary">Shop the Tee <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4"><polyline points="9 18 15 12 9 6" /></svg></span>
      ...
      <style>{`
        ...
        @media (max-width: 800px) { .featured-hero-card { grid-template-columns: 1fr !important; } .featured-hero-card > div:last-child { padding: 28px 28px 32px !important; } .featured-hero-card > div:last-child h2 { font-size: 32px !important; } }
      `}</style>
    </Link>
  );
}
  ...
  return (
    <section style={{ background: "var(--rut-lavender-bg)", borderRadius: 32, padding: "56px 48px", margin: "20px 0 0", border: "1px solid var(--rut-lavender-border)" }}>
      <div style={{ textAlign: "center", marginBottom: 36 }}>
        ...
        <h2 className="h-display" style={{ fontSize: 40, marginTop: 10 }}>Every Item Printed <span className="gradient-text">By Us</span></h2>
      </div>
